...
void vectorField::setupField(std::vector <ofTTFCharacter> paths, int innerW, int innerH, int outerW, int outerH){
    fieldWidth			= innerW;
    fieldHeight			= innerH;
    externalWidth		= outerW;
    externalHeight		= outerH;
    ...
    for (int i = 0; i<50; i++) {
        ...
        for(int k = 0; k < polylines.size(); k++) {
            ...
            for(int j = 1; j < vs.size(); j++) {
                ...
                ofPoint p2 = vs[j];
                printf("%f,%f\n", p2.x, p2.y);
                int fieldPosX = (int)((float)fieldWidth * p2.x / (float)externalWidth);
                int fieldPosY = (int)((float)fieldHeight * p2.y / (float)externalHeight);
                if (fieldPosX < 0 || fieldPosY < 0 || fieldPosX > fieldWidth-1 || fieldPosY > fieldHeight) {
                    continue;
                }
                int pos = fieldPosY * fieldWidth + fieldPosX;
                field[pos] = ofVec2f(p2.x - p1.x, p2.y - p1.y);
                printf("setup field %d,%d\n", fieldPosX, fieldPosY);
            }
        }
    }
}
